// TWW Season 3 전사 스킬 데이터
// The War Within 11.2.5 - 시즌 3 (2025-11-28 업데이트)
// PvP 특성 제외, 현재 활성화된 스킬만 포함
// 패시브 특성 포함
// 📌 가이드 작성 전 필수 참조: C:/wowmeta/WoW-Meta-Knowledge/99-META/Guide-Generation-Instructions.md

export const wowdbWarriorSkillsComplete = {
  "baseline": {
    "100": { "name": "Charge", "kr": "돌진", "level": 1, "type": "ability" },
    "355": { "name": "Taunt", "kr": "도발", "level": 1, "type": "ability" },
    "1160": { "name": "Demoralizing Shout", "kr": "사기의 외침", "level": 1, "type": "ability" },
    "1464": { "name": "Slam", "kr": "적중", "level": 1, "type": "ability" },
    "2565": { "name": "Shield Block", "kr": "방패 막기", "level": 1, "type": "ability" },
    "6343": { "name": "Thunder Clap", "kr": "천둥벼락", "level": 1, "type": "ability" },
    "6673": { "name": "Battle Shout", "kr": "전투의 외침", "level": 1, "type": "ability" },
    "18499": { "name": "Berserker Rage", "kr": "광전사의 격노", "level": 1, "type": "ability" }
  },
  "arms": {
    "772": { "name": "Rend", "kr": "분쇄", "level": 10, "type": "ability" },
    "12294": { "name": "Mortal Strike", "kr": "필사의 일격", "level": 10, "type": "ability" },
    "152277": { "name": "Ravager", "kr": "쇄도", "level": 10, "type": "ability" },
    "167105": { "name": "Colossus Smash", "kr": "거인의 강타", "level": 10, "type": "ability" },
    "227847": { "name": "Bladestorm", "kr": "칼폭풍", "level": 10, "type": "ability" }
  },
  "protection": {
    "871": { "name": "Shield Wall", "kr": "방패의 벽", "level": 10, "type": "ability" },
    "6572": { "name": "Revenge", "kr": "복수", "level": 10, "type": "ability" },
    "12975": { "name": "Last Stand", "kr": "최후의 저항", "level": 10, "type": "ability" },
    "23922": { "name": "Shield Slam", "kr": "방패 밀쳐내기", "level": 10, "type": "ability" }
  },
  "fury": {
    "1719": { "name": "Recklessness", "kr": "무모한 희생", "level": 10, "type": "ability" },
    "23881": { "name": "Bloodthirst", "kr": "피의 갈증", "level": 10, "type": "ability" },
    "85288": { "name": "Raging Blow", "kr": "성난 타격", "level": 10, "type": "ability" },
    "184367": { "name": "Rampage", "kr": "격앙", "level": 10, "type": "ability" }
  },
  "heroTalents": {
    "436147": { "name": "Thunder Blast", "kr": "천둥 폭발", "tree": "Mountain Thane", "level": 70, "type": "heroTalent" },
    "436152": { "name": "Crashing Thunder", "kr": "격돌하는 천둥", "tree": "Mountain Thane", "level": 70, "type": "heroTalent" },
    "440989": { "name": "Colossal Might", "kr": "거대한 힘", "tree": "Colossus", "level": 70, "type": "heroTalent" },
    "440993": { "name": "Demolish", "kr": "파괴", "tree": "Colossus", "level": 70, "type": "heroTalent" },
    "444770": { "name": "Slayer's Dominance", "kr": "학살자의 지배", "tree": "Slayer", "level": 70, "type": "heroTalent" },
    "444775": { "name": "Overwhelming Blades", "kr": "압도적인 칼날", "tree": "Slayer", "level": 70, "type": "heroTalent" }
  },
  "talents": {
    "12328": { "name": "Sweeping Strikes", "kr": "휩쓸기 일격", "type": "passive" },
    "29725": { "name": "Sudden Death", "kr": "급작스런 죽음", "type": "passive" },
    "58351": { "name": "Blood and Thunder", "kr": "피와 천둥", "type": "passive" },
    "103827": { "name": "Double Time", "kr": "이중 시간", "type": "passive" },
    "202095": { "name": "Vengeance", "kr": "복수심", "spec": "protection", "type": "passive" },
    "202751": { "name": "Anger Management", "kr": "분노 조절", "type": "passive" },
    "203177": { "name": "Heavy Repercussions", "kr": "무거운 반격", "spec": "protection", "type": "passive" },
    "206315": { "name": "Massacre", "kr": "대학살", "spec": "fury", "type": "passive" },
    "206333": { "name": "Taste for Blood", "kr": "피의 맛", "spec": "fury", "type": "passive" },
    "260708": { "name": "Sweeping Strikes", "kr": "휩쓸기 일격", "spec": "arms", "type": "passive" },
    "262111": { "name": "Mastery: Deep Wounds", "kr": "특화: 깊은 상처", "spec": "arms", "type": "passive" },
    "262150": { "name": "Frothing Berserker", "kr": "광분하는 광전사", "type": "passive" },
    "280392": { "name": "Meat Cleaver", "kr": "고기 절단기", "type": "passive" },
    "280721": { "name": "Sudden Death", "kr": "급작스런 죽음", "spec": "fury", "type": "passive" },
    "316402": { "name": "Martial Prowess", "kr": "무술 기량", "spec": "arms", "type": "passive" },
    "316428": { "name": "Revenge!", "kr": "복수!", "spec": "protection", "type": "passive" },
    "382940": { "name": "Punish", "kr": "응징", "spec": "protection", "type": "passive" },
    "382946": { "name": "Furious Bloodthirst", "kr": "격노한 피의 갈증", "spec": "fury", "type": "passive" },
    "383103": { "name": "Fueled by Violence", "kr": "폭력에 의한 연료", "type": "passive" },
    "383295": { "name": "Valor in Victory", "kr": "승리의 용맹", "spec": "arms", "type": "passive" },
    "383433": { "name": "Berserker's Might", "kr": "광전사의 힘", "spec": "fury", "type": "passive" },
    "384100": { "name": "Berserker Stance", "kr": "광전사 태세", "type": "passive" },
    "384318": { "name": "Desperate Strength", "kr": "필사적인 힘", "type": "passive" },
    "385391": { "name": "Improved Block", "kr": "방패 막기 연마", "spec": "protection", "type": "passive" },
    "385704": { "name": "Violent Outburst", "kr": "폭력적 분출", "spec": "protection", "type": "passive" },
    "385840": { "name": "Single-Minded Fury", "kr": "한손 무기 분노", "spec": "fury", "type": "passive" },
    "386196": { "name": "Berserker's Torment", "kr": "광전사의 고통", "type": "passive" }
  }
};

export function getWowDBWarriorSkill(skillId) {
  const categories = Object.keys(wowdbWarriorSkillsComplete);
  for (const category of categories) {
    if (wowdbWarriorSkillsComplete[category]?.[skillId]) {
      return { ...wowdbWarriorSkillsComplete[category][skillId], category, id: skillId };
    }
  }
  return null;
}

export function getWarriorSkillCount() {
  return Object.values(wowdbWarriorSkillsComplete).reduce((count, category) => 
    count + Object.keys(category).length, 0);
}

export function getWarriorSkillsBySpec(spec) {
  const specKey = spec?.toLowerCase().replace(/\s+/g, '');
  if (!wowdbWarriorSkillsComplete[specKey]) return [];
  return Object.entries(wowdbWarriorSkillsComplete[specKey]).map(([id, skill]) => ({
    ...skill, id: parseInt(id), category: specKey
  }));
}
