// TWW Season 3 도적 스킬 데이터
// The War Within 11.2.5 - 시즌 3 (2025-11-28 업데이트)
// PvP 특성 제외, 현재 활성화된 스킬만 포함
// 패시브 특성 포함
// 📌 가이드 작성 전 필수 참조: C:/wowmeta/WoW-Meta-Knowledge/99-META/Guide-Generation-Instructions.md

export const wowdbRogueSkillsComplete = {
  "baseline": {
    "408": { "name": "Kidney Shot", "kr": "급소 가격", "level": 1, "type": "ability" },
    "1752": { "name": "Sinister Strike", "kr": "사악한 일격", "level": 1, "type": "ability" },
    "1856": { "name": "Vanish", "kr": "소멸", "level": 1, "type": "ability" },
    "1966": { "name": "Feint", "kr": "교란", "level": 1, "type": "ability" },
    "2983": { "name": "Sprint", "kr": "전력 질주", "level": 1, "type": "ability" },
    "185311": { "name": "Crimson Vial", "kr": "진홍색 약병", "level": 1, "type": "ability" },
    "196819": { "name": "Eviscerate", "kr": "절개", "level": 1, "type": "ability" }
  },
  "assassination": {
    "703": { "name": "Garrote", "kr": "목조르기", "level": 10, "type": "ability" },
    "1329": { "name": "Mutilate", "kr": "훼손", "level": 10, "type": "ability" },
    "1943": { "name": "Rupture", "kr": "파열", "level": 10, "type": "ability" },
    "32645": { "name": "Envenom", "kr": "독살", "level": 10, "type": "ability" }
  },
  "outlaw": {
    "13750": { "name": "Adrenaline Rush", "kr": "아드레날린 촉진", "level": 10, "type": "ability" },
    "193315": { "name": "Roll the Bones", "kr": "뼈 주사위", "level": 10, "type": "ability" },
    "315508": { "name": "Roll the Bones", "kr": "뼈 주사위", "level": 10, "type": "ability" }
  },
  "subtlety": {
    "121471": { "name": "Shadow Blades", "kr": "그림자 칼날", "level": 10, "type": "ability" },
    "185438": { "name": "Shadowstrike", "kr": "그림자 일격", "level": 10, "type": "ability" },
    "280719": { "name": "Secret Technique", "kr": "비밀 기술", "level": 10, "type": "ability" }
  },
  "heroTalents": {
    "441403": { "name": "Unseen Blade", "kr": "보이지 않는 칼날", "tree": "Trickster", "level": 70, "type": "heroTalent" },
    "452068": { "name": "Hand of Fate", "kr": "운명의 손", "tree": "Fatebound", "level": 70, "type": "heroTalent" },
    "457033": { "name": "Hunt Them Down", "kr": "사냥 개시", "tree": "Deathstalker", "level": 70, "type": "heroTalent" }
  },
  "talents": {
    "2823": { "name": "Deadly Poison", "kr": "맹독", "type": "passive" },
    "5171": { "name": "Slice and Dice", "kr": "난도질", "type": "passive" },
    "14161": { "name": "Ruthlessness", "kr": "무자비", "type": "passive" },
    "14190": { "name": "Improved Sprint", "kr": "전력 질주 연마", "type": "passive" },
    "31209": { "name": "Master Poisoner", "kr": "독의 대가", "type": "passive" },
    "79140": { "name": "Vendetta", "kr": "원한", "type": "passive" },
    "193640": { "name": "Elaborate Planning", "kr": "정교한 계획", "type": "passive" },
    "196819": { "name": "Envenom", "kr": "독살", "spec": "assassination", "type": "passive" },
    "196937": { "name": "Ghostly Strike", "kr": "유령의 일격", "spec": "outlaw", "type": "passive" },
    "196951": { "name": "Master of Shadows", "kr": "그림자 숙련", "spec": "subtlety", "type": "passive" },
    "200806": { "name": "Exsanguinate", "kr": "방혈", "spec": "assassination", "type": "passive" },
    "245388": { "name": "Toxic Blade", "kr": "맹독 칼날", "spec": "assassination", "type": "passive" },
    "271877": { "name": "Blade Rush", "kr": "칼날 질풍", "spec": "outlaw", "type": "passive" },
    "277925": { "name": "Shuriken Tornado", "kr": "수리검 회오리", "spec": "subtlety", "type": "passive" },
    "280719": { "name": "Secret Technique", "kr": "비밀 기술", "spec": "subtlety", "type": "passive" },
    "315341": { "name": "Between the Eyes", "kr": "미간 사격", "spec": "outlaw", "type": "passive" },
    "381623": { "name": "Thistle Tea", "kr": "가시엉겅퀴 차", "type": "passive" },
    "381989": { "name": "Keep it Rolling", "kr": "계속 굴려라", "spec": "outlaw", "type": "passive" },
    "385125": { "name": "Shadowcraft", "kr": "그림자 세공", "spec": "subtlety", "type": "passive" },
    "385627": { "name": "Kingsbane", "kr": "왕의 파멸", "spec": "assassination", "type": "passive" }
  }
};

export function getWowDBRogueSkill(skillId) {
  const categories = Object.keys(wowdbRogueSkillsComplete);
  for (const category of categories) {
    if (wowdbRogueSkillsComplete[category]?.[skillId]) {
      return { ...wowdbRogueSkillsComplete[category][skillId], category, id: skillId };
    }
  }
  return null;
}
