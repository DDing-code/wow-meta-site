...
function displayGuideText(value) {
  return normalizeCommunityTerms(cleanText(value)
    ...
    .replace(/대상망으로/g, '대상 배치로')
    .replace(/대상망을/g, '대상 배치를')
    .replace(/대상망이/g, '대상 배치가')
    .replace(/대상망과/g, '대상 배치와')
    .replace(/대상망/g, '대상 배치')
    .replace(/회로/g, '흐름')
    .replace(/\(KST\)/g, '(한국 시간)')
...
function getInlineChartPlan(guide, data) {
  ...
  if (guide.id === 'monk-mistweaver') {
    plan.push({
      ...
      sectionIntro:
        '운무 수도사는 소생의 안개로 생기 충전 대상 배치를 만들고, 피해 예고에 맞춰 생기 충전, 마나 차, 천신합일, 재활, 기의 고치를 분산하는 힐러입니다.',
      caption:
        '이 막대는 HPS 수치표가 아니라 소생의 안개 대상 유지, 생기 충전 복구, 근접 치유 엔진, 마나 차, 단일 대상 외부 생존기, 대형 공대 쿨기를 한눈에 모은 힐러 흐름도입니다.',
      definition: [
        ['의미', '소생의 안개는 생기 충전이 여러 대상에게 퍼질 준비를 만들고, 생기 충전은 피해가 들어온 뒤 그 대상 배치를 통해 회수하는 핵심 주문입니다.'],
        ['읽는 법', '피해 전에는 소생의 안개와 마나 차를 준비하고, 피해 직후에는 생기 충전 또는 재활/천신합일을 배정하며, 단일 대상 위험은 포용의 안개와 기의 고치로 분리합니다.'],
        ['체크 포인트', '소생의 안개 충전 과충전, 생기 충전 대상 배치 부족, 마나 차 스택 방치, 질풍차기 근접 공백, 재활/천신합일 중복, 기의 고치 지연을 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warrior-protection') {
    plan.push({
      ...
      caption:
        '막대는 실제 WCL 수치를 복사한 것이 아니라 전투 분석 순서를 시각화한 것입니다. 먼저 방패 올리기 공백을 보고, 그 공백에 고통 감내·주문 반사·사기의 외침·방패의 벽이 덮였는지 확인한 뒤 산왕 또는 거신 피해 구간을 읽습니다.',
      definition: [
  ...
  if (guide.id === 'warrior-arms') {
    plan.push({
      ...
      definition: [
        ['의미', '필사의 일격은 중심 타격이고, 거인의 강타와 전쟁파괴자는 그 타격과 후속 고가치 기술을 몰아넣는 피해 구간입니다.'],
  ...
  if (guide.id === 'shaman-restoration') {
    plan.push({
      ...
      sectionIntro:
        '복원 주술사는 고정 딜사이클이 아니라 피해 예고에 맞춰 성난 해일 대상 배치, 치유의 비/쇄도하는 토템 위치, 폭우와 연쇄 치유 회수, 치유의 해일 토템/승천/정신의 고리 토템 배정을 겹치는 힐러입니다.',
      caption:
        '막대는 실제 HPS 초 단위 복사본이 아니라 로그 확인 순서입니다. 먼저 성난 해일 대상 배치와 위치 기반 힐을 보고, 그 다음 폭우/연쇄 치유 회수, 큰 공대 쿨기, 차단/해제를 확인합니다.',
      definition: [
  ...
  if (guide.id === 'paladin-holy') {
    plan.push({
      ...
      sectionHeading: '신성 충격과 봉화 대상 배치',
      sectionIntro:
        '신성 성기사는 신성 충격으로 신성한 힘과 빛 주입을 만들고, 봉화 대상 배치로 치유를 분배한 뒤, 단일 위험과 광역 피해에 서로 다른 소비기와 쿨다운을 배정하는 힐러입니다.',
      caption:
        '막대는 실제 HPS 복사본이 아니라 신성 충격 충전, 빛 주입 소비, 빛의 봉화/구세주의 봉화 대상 배치, 영광의 서약/영원의 불꽃 단일 회복, 여명의 빛 광역 회복, 오라 숙련과 티르의 해방 선배정을 한 화면에 묶은 확인용 흐름도입니다.',
      definition: [
        ['의미', '신성 충격은 신성 성기사의 중심 생성기이고, 봉화 대상 배치는 그 치유가 실제로 도착하는 경로입니다.'],
  ...
  if (guide.id === 'paladin-retribution') {
    plan.push({
      ...
      definition: [
        ['의미', '사형 선고는 징벌의 피해 몰아넣기 기준이고, 신성한 힘은 그 구간에 고가치 소모기를 넣기 위해 비워 두는 자원입니다.'],
  ...
  if (guide.id === 'druid-restoration') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '피해 후 회복 난사, 피어나는 생명 공백, 꽃피우기 위치 이탈, 신속한 치유 후속 주문 누락, 평온 이동 끊김을 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'druid-guardian') {
    plan.push({
      ...
      caption: '막대는 실제 HPS나 DPS 수치가 아니라 달빛섬광 풀링, 난타 위협, 무쇠가죽 선적용, 달 광선 위치 고정, 생존기/차단 분기를 한 화면에 놓는 전투 흐름도입니다.',
      definition: [
  ...
  if (guide.id === 'druid-feral') {
    plan.push({
      ...
      definition: [
        ['의미', '레이드 야생추적자는 갈퀴 발톱과 도려내기 품질을, 쐐기 발톱의 드루이드는 출혈 기반 위에 직접 피해 구간을 얹는지를 확인합니다.'],
  ...
  if (guide.id === 'priest-discipline') {
    return [
      ...
      },
      {
        ...
        note: '속죄가 남아 있는 동안 실제 치유량을 돌려주는 피해 주문 축입니다. 속죄가 없으면 가치가 크게 줄어듭니다.',
      ...
      },
      {
        ...
        note: '현재 레이드/쐐기 로그의 기본 영웅 특성 축입니다. 회개 보강과 보호막 보조로 큰 쿨다운 사이 빈 구간을 메웁니다.',
    ...
    return [
      {
        ...
        note: '비전 탄막 소비 품질을 결정하므로 큰 구간 전 예열과 구간 안 소비를 같이 봅니다.',
  ...
  if (guide.id === 'shaman-elemental') {
    return [
      ...
      },
      {
        ...
        note: 'Archon 최근 로그의 기본 영웅 특성 축입니다. 폭풍 가능 상태를 방치하지 않습니다.',
    ...
    return [
      {
        ...
        note: '대상 배치, 굽이치는 물결, 선조 발동, 연쇄 치유 첫 대상을 묶는 중앙 노드입니다.',
      ...
      },
      {
        ...
        note: '성난 해일 대상 배치와 거리 조건이 맞을 때 가장 먼저 가치가 올라갑니다.',
  ...
  if (guide.id === 'rogue-outlaw') {
    return [
      ...
      },
      {
        ...
        note: '쿨다운 손실을 막아야 하는 고가치 마무리 기술입니다.',
    ...
    return [
      {
        ...
        note: '생기 충전 확산의 대상 배치입니다. 충전이 넘치기 전에 피해 전 대상 수를 확보합니다.',
        segments: [[3, 28], [35, 28], [68, 26]],
      },
      {
        label: '피해 복구',
        skill: findSkillByNames(data, ['생기 충전']),
        note: '피해 직후 소생의 안개 대상 배치를 통해 광역 회복을 회수합니다.',
  ...
  if (guide.id === 'paladin-holy') {
    return [
      ...
      },
      {
        ...
        note: '태양의 사자에서 새벽빛과 태양의 화신 대상 배치를 여는 짧은 쿨다운입니다.',
