...
function getInlineChartPlan(guide, data) {
  ...
  if (guide.id === 'warlock-destruction') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '제물/쇠퇴 공백, 점화 2충전 방치, 역류가 긴 시전과 어긋난 구간, 대혼란 안 빈 창, 불의 비 적중 시간 부족을 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warrior-arms') {
    plan.push({
      ...
      definition: [
        ['의미', '필사의 일격은 중심 타격이고, 거인의 강타와 전쟁파괴자는 그 타격과 후속 고가치 기술을 압축하는 피해 창입니다.'],
  ...
  if (guide.id === 'warrior-fury') {
    plan.push({
      ...
      caption:
        '막대는 실제 WCL 초 단위 복사본이 아니라 로그를 확인하는 순서입니다. 광란 지연과 격노 공백을 먼저 보고, 그 아래 피의 갈증, 분노의 강타, 무모한 희생, 투신, 우레 작렬, 소용돌이 연마, 학살자 마무리 일격/칼날폭풍이 같은 창에 들어왔는지 확인합니다.',
      definition: [
        ...
        ['체크 포인트', '광란 지연, 격노가 창 안에서 빠진 시간, 피의 갈증/분노의 강타 충전 낭비, 우레 작렬 과충전, 소용돌이 연마 없는 광역 광란, 학살자 칼날폭풍 지연을 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'shaman-elemental') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '화염 충격 공백, 용암 폭발 충전 낭비, 소용돌이 과충전, 폭풍수호자 강화 주문 미소비, 폭풍 방치, 이동 중 큰 창 손실, 날카로운 바람 누락을 함께 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'shaman-enhancement') {
    plan.push({
      ...
      caption:
        '막대는 실제 WCL 초 단위 복사본이 아니라 로그 확인 순서입니다. 먼저 소용돌이치는 무기 과충전과 소비 간격을 보고, 그 다음 폭풍의 일격/용암 채찍 환급, 낙뢰 광역 전환, 쇄도하는 토템 또는 폭풍 창을 확인합니다.',
      definition: [
  ...
  if (guide.id === 'demonhunter-havoc') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '복수의 퇴각과 지옥칼은 피해 증폭 수단이지만, 안전한 복귀 경로가 없으면 창 자체가 실패합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'deathknight-blood') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '죽음과 부패 위치 손실, 죽음의 일격 선소모, 룬 무기 창 안 빈 전역, 원거리 몹 정리 실패를 함께 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'druid-balance') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '일월식 2충전 방치, 천공의 힘 과충전, 별재봉사/우주의 손길 발동 방치, 도트 창 내부 갱신, 쐐기 풀링 위협을 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'mage-fire') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '발화 지연, 유성 착탄 이탈, 화염 작렬 과충전, 몰아치는 열기! 방치, 불기둥 대상 수 부족, 이동으로 인한 발화 창 손실을 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warlock-demonology') {
    return [
      ...
      },
      {
        ...
        note: '다중 대상 또는 큰 창에서 별도 피해 축으로 확인합니다.',
  ...
  if (guide.id === 'paladin-protection') {
    return [
      ...
      },
      {
        ...
        note: '기사단은 큰 공격 창, 빛대장장이는 보호/지원 창으로 나눠 확인합니다.',
