...
function getInlineChartPlan(guide, data) {
  ...
  if (guide.id === 'monk-mistweaver') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '소생의 안개 충전 과충전, 생기 충전 대상망 부족, 마나 차 스택 방치, 질풍차기 근접 공백, 재활/천신합일 중복, 기의 고치 지연을 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warrior-arms') {
    plan.push({
      ...
      definition: [
        ...
        ['체크 포인트', '필사의 일격 지연, 거인의 강타 안 빈 전역, 제압 2충전 방치, 처형 구간 분노 고갈, 쇄파 위치 손실, 학살자 광역 제압 우선순위 오류를 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'shaman-restoration') {
    plan.push({
      ...
      caption:
        '막대는 실제 HPS 초 단위 복사본이 아니라 로그 확인 순서입니다. 먼저 성난 해일 대상망과 위치 기반 힐을 보고, 그 다음 폭우/연쇄 치유 회수, 큰 공대 쿨기, 차단/해제를 확인합니다.',
      definition: [
  ...
  if (guide.id === 'paladin-holy') {
    plan.push({
      ...
      caption:
        '막대는 실제 HPS 복사본이 아니라 신성 충격 충전, 빛 주입 소비, 빛의 봉화/구세주의 봉화 대상망, 영광의 서약/영원의 불꽃 단일 회복, 여명의 빛 광역 회복, 오라 숙련과 티르의 해방 선배정을 한 화면에 묶은 확인용 흐름도입니다.',
      definition: [
