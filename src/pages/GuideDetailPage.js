...
function getInlineChartPlan(guide) {
  ...
  if (guide.id === 'warrior-arms') {
    plan.push({
      ...
      caption:
        '막대는 실제 WCL 초 단위 복사본이 아니라 로그를 읽는 순서입니다. 필사의 일격 지연을 먼저 보고, 그 앞뒤로 거인의 강타/전쟁파괴자, 제압 충전, 분쇄/치명상, Slayer 칼날폭풍, Colossus 쇄파가 맞물렸는지 확인합니다.',
      definition: [
        ...
        ['검수 포인트', '필사의 일격 지연, 거인의 강타 안 빈 전역, 제압 2충전 방치, 처형 구간 분노 고갈, 쇄파 위치 손실, Slayer 광역 제압 우선순위 오류를 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warrior-fury') {
    plan.push({
      ...
      sectionIntro:
        '분노 전사는 광란을 중심으로 격노 상태, 분노 생성기, 무모한 희생/투신 창, Slayer 또는 Mountain Thane 분기를 동시에 읽어야 합니다. 이 타임라인은 광란이 언제 단순 소비기가 아니라 상태 전환과 광역 발동 조건이 되는지 보여줍니다.',
      caption:
        '막대는 실제 WCL 초 단위 복사본이 아니라 로그를 검수하는 순서입니다. 광란 지연과 격노 공백을 먼저 보고, 그 아래 피의 갈증, 분노의 강타, 무모한 희생, 투신, 우레 작렬, 소용돌이 연마, Slayer 마무리 일격/칼날폭풍이 같은 창에 들어왔는지 확인합니다.',
      definition: [
        ['의미', '광란은 분노를 피해로 바꾸는 버튼이면서 격노를 켜고 12.0.5 Slayer 광역 발동까지 연결하는 중심 노드입니다.'],
        ['읽는 법', '상단의 광란 주기를 기준으로 격노 공백, 분노 과충전, 생성기 지연, 무모한 희생/투신 창, 우레 작렬 2중첩, 소용돌이 연마 공백을 차례대로 봅니다.'],
        ['검수 포인트', '광란 지연, 격노가 창 안에서 빠진 시간, 피의 갈증/분노의 강타 충전 낭비, 우레 작렬 과충전, 소용돌이 연마 없는 광역 광란, Slayer 칼날폭풍 지연을 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'demonhunter-devourer') {
    plan.push({
      ...
      caption: '막대는 실제 로그 수치가 아니라 흡수 자원 램프, 공허 광선, 수확/박멸 반응, 공허 탈태 진입, 붕괴하는 별 내부 소비를 한 화면에 묶은 상태 전환 도식입니다.',
      definition: [
  ...
  if (guide.id === 'deathknight-frost') {
    plan.push({
      ...
      caption: '막대는 실제 DPS 수치가 아니라 얼음 기둥 안에 들어갈 자원, 발동, 후속 회수를 한 화면에 놓는 운용 도식입니다.',
      definition: [
        ['의미', '얼음 기둥을 기준으로 사신의 징표와 룬 무기 강화, 도살기 소비, 신드라고사의 숨결 유지 여부를 함께 봅니다.'],
        ['읽는 법', '단일은 도살기 절멸, 3타겟 이상은 도살기 서리낫, 숨결 빌드는 룬 마력 유지 곡선을 우선합니다.'],
        ['검수 포인트', '사신의 징표 대상 생존, 룬 무기 강화 2충전 방치, 도살기 2중첩 손실, 단단한 얼음 미소비 시간을 함께 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'deathknight-blood') {
    plan.push({
      ...
      definition: [
        ['의미', '피해 전에는 뼈의 보호막과 생존기 배정을 보고, 피해 직후에는 죽음의 일격을 쓸 룬 마력이 남았는지 확인합니다.'],
        ['읽는 법', '춤추는 룬 무기와 흡혈은 같은 피해에 무조건 겹치기보다 다음 위험 구간 공백을 기준으로 나눕니다.'],
        ['검수 포인트', '산레인은 룬 무기/흡혈 창, Deathbringer는 사신의 징표 대상 선택과 쐐기 풀 안정화까지 함께 봅니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'druid-balance') {
    plan.push({
      id: 'uptime',
      title: '일월식-소비기 체크라인',
      sectionHeading: '일월식 창과 천공의 힘',
      sectionIntro:
        '조화는 일월식 자체보다 일월식 초반에 어떤 소비기와 준비 버튼이 들어갔는지가 중요합니다.',
      caption:
        '막대는 실제 WCL 유지율이 아니라 일월식 충전, 도트 갱신, 천공의 힘, 무료 소비기, 큰 쿨다운을 한 화면에 놓는 판단 도식입니다.',
      definition: [
        ['의미', '일월식은 유지 버프가 아니라 별빛쇄도와 별똥별을 넣기 위한 짧은 피해 창입니다.'],
        ['읽는 법', '도트와 준비 버튼은 창 전에, 소비기와 영혼 소집은 창 안에 들어가도록 봅니다.'],
        ['검수 포인트', '일월식 2충전 방치, 천공의 힘 과충전, 별재봉사/우주의 손길 발동 방치, 도트 창 내부 갱신을 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'druid-guardian') {
    plan.push({
      ...
      definition: [
        ['의미', '무쇠가죽은 물리 피해 전에, 광포한 재생력은 피해 후 복구에, 나무 껍질과 생존 본능은 큰 피해 구간에 배정합니다.'],
        ['읽는 법', '엘룬의 선택은 난타/달빛섬광으로 달 광선을 빨리 돌리고, 발톱의 드루이드는 찢어발기기/말살을 방어 예산이 남을 때 소비합니다.'],
        ['검수 포인트', '사망 직전 무쇠가죽 공백, 광포한 재생력 과잉 회복, 달 광선 위치 손실, 공격 소비기가 방어 분노를 먹은 구간을 함께 확인합니다.'],
      ],
    });
    return plan;
  }
  ...
  if (guide.id === 'warrior-fury') {
    return [
      ...
      },
      {
        ...
        note: 'Mountain Thane은 우레 작렬 2중첩과 천둥벼락 초기화를 함께 봅니다.',
      ...
      },
      {
        ...
        note: '우레 작렬 2중첩 과충전과 투신 창의 천둥벼락 전환을 추적합니다.',
