 ...
 *    - 키 이름: 'mountainthane'/'rideroftheapocalypse' → 실제 영웅특성 영문명
 *    - name/icon/tierSet/opener/priority 모두 교체
 *
 * 4. 영웅특성 선택 버튼 수정 (검색: "setSelectedTier")
 *    - setSelectedTier('mountainthane') → 실제 영웅특성명
 *
...
import { frostDeathKnightSkills as skillData} from '../data/frostDeathKnightSkillData';
...
const HeroCard = styled(Card)`
  background: ${props => {
    if (props.heroType === 'mountainthane') {
...
// - 키 이름: 'mountainthane', 'rideroftheapocalypse' → 실제 영웅특성 영문명 (예: 'frostfire', 'farseer')
// - name, icon, tierSet, opener, priority 모두 교체
// - 영웅특성별로 단일/광역 우선순위가 다르므로 각각 작성
const getHeroContent = (SkillIcon) => ({
  mountainthane: {  // ⚠️ TODO: 실제 영웅특성 영문명으로 변경 (예: frostfire, farseer)
    name: '산왕',  // ⚠️ TODO: 실제 영웅특성 한글명
    ...
    singleTarget: {
      opener: [
        ...
        skillData.empowerruneweapon,         // 전투 3초 전: 환기 (완전 충전)
        skillData.howlingblast,    // Pull 시작: 신비한 화살
        skillData.pillarofthe,          // Time Warp (팀 공속 버프)
        skillData.breathofthe,       // Arcane Surge (주요 버스트 쿨다운)
        skillData.frostscythe,    // Touch of the Magi (탄막 비행 중)
        skillData.glacialadvance,         // 비전 보주 (충전물 생성)
        skillData.obliterate,       // 비전 작렬
        skillData.obliterate,       // 비전 작렬
        skillData.obliterate,       // 비전 작렬 (4충전)
        skillData.froststr,     // 비전 탄막 (4충전 소모)
        skillData.obliterate        // 사이클 계속
      ],
      priority: [
        {
    ...
    aoe: {
      opener: [
        skillData.raisedead,       // 전투 4초 전: 환영 복제 (DPS 증가 + 생존력)
        skillData.empowerruneweapon,         // 전투 3초 전: 환기 (완전 충전)
        skillData.glacialadvance,         // Pull 시작: 비전 보주
        skillData.pillarofthe,          // Time Warp (팀 공속 버프)
        skillData.breathofthe,       // Arcane Surge (주요 버스트 쿨다운)
        skillData.frostscythe,    // Touch of the Magi (탄막 비행 중)
        skillData.frostnova,   // 광역 피해
        skillData.glacialadvance,         // 비전 보주
        skillData.frostnova,   // 광역 피해
        skillData.obliterate,       // 충전 쌓기
        skillData.froststr,     // 비전 탄막 (광역)
  ...
  rideroftheapocalypse: {
    ...
    singleTarget: {
      opener: [
        ...
        skillData.empowerruneweapon,         // 전투 3초 전: 환기 (완전 충전)
        skillData.howlingblast,    // Pull 시작: 신비한 화살
        skillData.glacialadvance,         // Arcane Orb (High Voltage proc 있을 경우)
        skillData.pillarofthe,          // Time Warp (팀 공속 버프)
        skillData.breathofthe,       // Arcane Surge (주요 버스트 쿨다운)
        skillData.frostscythe,    // Touch of the Magi (탄막 비행 중)
        skillData.froststr,     // Arcane Barrage (4충전 - Intuition 트리거)
        skillData.howlingblast,    // Arcane Missiles (Clearcasting)
        skillData.obliterate,       // Arcane Blast (충전물 쌓기)
        skillData.obliterate,       // Arcane Blast
        skillData.obliterate,       // Arcane Blast
        skillData.obliterate        // Arcane Blast (4충전)
      ],
      priority: [
        {
          skill: skillData.froststr,
          ...
          priority: 0,
          why: '종말의 기수 핵심 - 4충전 탄막으로(로) 직관 (40% 확률) 및 비전의 박자 버프 발동/유지'
        },
        {
          skill: skillData.mindfreeze,
          desc: '힘의 전환 (전략적)',
    ...
    aoe: {
      opener: [
        ...
        skillData.obliterate,       // 충전 쌓기
        skillData.obliterate,       // 충전 쌓기
        skillData.froststr,     // 비전 탄막 (Intuition 트리거)
        skillData.frostnova    // 광역 계속
      ],
      priority: [
        {
    ...
    mechanics: [
      ...
      },
      {
        ...
        why: '종말의 기수 특화 - 번뜩임 + 비전 조화 중첩 관리가 핵심'
      },
      {
...
const ArcaneMageGuide = () => {
  ...
  const [selectedTier, setSelectedTier] = useState('mountainthane');
  const [showToast, setShowToast] = useState(false);
  const [showCopyToast, setShowCopyToast] = useState(false);
  const [selectedBuild, setSelectedBuild] = useState('raid-single');
  const [selectedStatHero, setSelectedStatHero] = useState('mountainthane');
  ...
  const renderTextWithSkillIcons = (text) => {
    ...
    const skillNameMap = {
      '비전 작렬': skillData.obliterate,
      '비전 탄막': skillData.froststr,
      '신비한 화살': skillData.howlingblast,
      '비전 보주': skillData.glacialadvance,
      '신비한 폭발': skillData.frostnova,
      '비전의 여파': skillData.frostscythe,
      '환기': skillData.empowerruneweapon,
      ...
      // 버프 및 메커니즘
      '비전의 영혼': skillData.remorselesswinter,
      '황천의 정밀함': skillData.frostfever,
      '번뜩임': skillData.killingmachine,
      '직관': skillData.rimestrike,
      '비전의 박자': skillData.frostfever,
      // 리소스 및 티어 세트
      '비전 충전물': skillData.runicpower,
      '마나': skillData.runicpower,
      '비전 조화': skillData.killingmachine,
      '주문불꽃 구체': skillData.gatheringstorm
    };
      ...
      <Card>
        <div className={styles.subsection} ref={subSectionRefs['overview-intro']}>
          ...
          <p style={{ marginBottom: '20px', lineHeight: '1.6' }}>
            ...
            현재 시즌에서는 <span style={{ color: '#FF6B6B', fontWeight: 'bold' }}>산왕</span>이 주류 빌드이며, 번뜩임 관리를 통한 주문불꽃 구체 극대화가 핵심입니다.
          </p>
          ...
          <p style={{ marginBottom: '20px', lineHeight: '1.8' }}>
            ...
            <SkillIcon skill={skillData.glacialadvance} textOnly={true} />로 비전 충전물을(를) 생성하고,
            <SkillIcon skill={skillData.froststr} textOnly={true} />로 폭발적인 딜을 냅니다.
          </p>
          <p style={{ marginBottom: '20px', lineHeight: '1.8' }}>
            번뜩임 프록 발동 시 <SkillIcon skill={skillData.howlingblast} textOnly={true} />를 즉시 시전하며,
            마나 소진 시에는
            <SkillIcon skill={skillData.empowerruneweapon} textOnly={true} />로 마나를 회복합니다.
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '15px', marginBottom: '30px' }}>
            {[
              ...
              { skill: skillData.howlingblast, label: '번뜩임 소비' },
              { skill: skillData.empowerruneweapon, label: '마나 회복' },
          ...
          <ul style={{ lineHeight: '1.8', marginBottom: '20px' }}>
            ...
            <li>충전물 생성:
              <ul style={{ marginTop: '0.5rem', paddingLeft: '2rem' }}>
                <li><SkillIcon skill={skillData.obliterate} textOnly={true} /> - 비전 충전물 1개 생성 (피해 +60%, 마나 +100%, 시전 시간 -8%)</li>
                <li><SkillIcon skill={skillData.glacialadvance} textOnly={true} /> - 비전 충전물 1개 생성 + 적 통과 시 추가 생성</li>
                <li><SkillIcon skill={skillData.frostnova} textOnly={true} /> - 적 타격 시 비전 충전물 1개 생성 (광역 전용)</li>
              </ul>
            </li>
            <li>충전물 소비:
              <ul style={{ marginTop: '0.5rem', paddingLeft: '2rem' }}>
            ...
            <li><strong style={{ color: '#ffa500' }}>핵심 전략:</strong> 4충전물 유지 → <SkillIcon skill={skillData.froststr} textOnly={true} />로 폭발적 피해 (마나 관리 필수)</li>
            <li><strong style={{ color: '#ff6b6b' }}>주의:</strong> 마나가 20% 이하로 떨어지면 <SkillIcon skill={skillData.empowerruneweapon} textOnly={true} />으로 즉시 회복해야 딜 공백 방지</li>
          </ul>

          <h4 style={{ color: '#ffa500', fontSize: '1.2rem', marginTop: '25px', marginBottom: '15px' }}>주요 메커니즘</h4>
          <ul style={{ lineHeight: '1.8', marginBottom: '20px' }}>
            <li><strong style={{ color: '#3FC6EA' }}>번뜩임:</strong> <SkillIcon skill={skillData.obliterate} textOnly={true} /> 시전 시 확률로 발동, 3중첩 번뜩임 시 <SkillIcon skill={skillData.howlingblast} textOnly={true} /> 즉시 시전 필수</li>
            <li><strong style={{ color: '#ff6b6b' }}>비전 조화 (종말의 기수):</strong> 신비한 화살 적중 시 피해 증가 중첩 (최대 20중첩 = 100% 피해 증가), 번뜩임과 함께 사용</li>
            <li><strong style={{ color: '#ffa500' }}>소진/보존 페이즈:</strong> 소진 페이즈(4충전물 유지) → 마나 30% 이하 → 보존 페이즈(저마나 스킬) → <SkillIcon skill={skillData.empowerruneweapon} textOnly={true} /> 회복</li>
            ...
            <li><strong style={{ color: '#32CD32' }}>영웅 특성 활용:</strong> 산왕은 번뜩임으로 주문화염 구체 극대화, 종말의 기수는 직관 관리 및 비전 조화 중첩 유지</li>
          </ul>
        </div>
      </Card>
    </Section>
      ...
      <HeroCard heroType={selectedTier}>
        <div className={styles.section}>
          ...
          <div className={styles.tierTabs} style={{ marginBottom: '30px' }}>
            ...
            >
              <span className={styles.tierIcon}>☀️</span> 산왕
            </button>
            <button
          ...
          <div className={styles.subsection} ref={subSectionRefs['rotation-tier']}>
            <h3 className={styles.subsectionTitle} style={{
              color: selectedTier === 'mountainthane' ? '#3FC6EA' : '#4ECDC4'
            }}>티어 세트 효과</h3>
            <div className={styles.tierBonuses} style={{
              ...
              padding: '1.5rem',
              borderRadius: '8px',
              border: selectedTier === 'mountainthane'
          ...
          <div className={styles.subsection} style={{
            ...
            {selectedTier === 'mountainthane' ? (
              <>
                <p style={{ marginBottom: '20px', lineHeight: '1.8' }}>
                  <strong style={{ color: '#FF8C42' }}>산왕</strong>은 {' '}
                  비전과 화염 마법의 융합을 통한 {' '}
              ...
              <>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <p style={{ color: '#ffa500', fontSize: '0.95rem', fontWeight: 'bold' }}>
                    💡 프로 팁: {renderTextWithSkillIcons('비전 보주는 티어 4세트와 함께 사용 시 빠르게 4중첩을 만들 수 있어 종말의 기수의 핵심 스킬입니다.')}
                  </p>
                </div>
            ...
            <div style={{
              ...
              <p style={{ color: selectedTier === 'mountainthane' ? '#3FC6EA' : '#4ECDC4', fontSize: '0.95rem', margin: 0 }}>
          ...
          <div className={styles.subsection} ref={subSectionRefs['rotation-aoe']}>
            <h3 className={styles.subsectionTitle} style={{
              color: selectedTier === 'mountainthane' ? '#9482C9' : '#32CD32',
              marginTop: '1.5rem'
            }}>광역 대상 (4+ 타겟)</h3>
            ...
            {selectedTier === 'mountainthane' && (
              <>
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    <li>
                      <strong style={{ color: '#ffa500' }}>핵심 메커니즘:</strong> 산왕의 전체 플레이스타일은 주문불꽃 구체를 최대한 많이 생성하는 것
                    </li>
                    <li>
                      <strong>소비 방법:</strong> <SkillIcon skill={skillData.breathofthe} textOnly={true} />을 통해 소비
                    </li>
                    <li>
                      <strong style={{ color: '#ff6b6b' }}><SkillIcon skill={skillData.pillarofthe} textOnly={true} />:</strong> 주문불꽃 구체 소비 시 비전 충전물 즉시 재생성
                    </li>
                    <li>
                      <strong>생성 조건:</strong> 비전 주문 시전 시 일정 확률로 생성 (<SkillIcon skill={skillData.breathofthe} textOnly={true} /> 사용 시 대량 생성)
                    </li>
                    <li>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    <li>
                      <strong style={{ color: '#ffa500' }}><SkillIcon skill={skillData.breathofthe} textOnly={true} />:</strong> 주요 쿨다운 - 주문력 대폭 증가 + 마나 100% 회복 + <SkillIcon skill={skillData.killingmachine} textOnly={true} /> 자동 부여
                    </li>
                    <li>
                      <strong><SkillIcon skill={skillData.frostscythe} textOnly={true} />:</strong> 12초 동안 대상에게 입힌 피해의 20% 누적 → 폭발 시 비전 피해
                    </li>
                    <li>
                    ...
                    <li>
                      <strong>버스트 순서:</strong> <SkillIcon skill={skillData.frostscythe} textOnly={true} /> → <SkillIcon skill={skillData.breathofthe} textOnly={true} /> → 비전 충전물 4중첩 유지 → <SkillIcon skill={skillData.froststr} textOnly={true} /> 폭발
                    </li>
                    <li>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    ...
                    <li>
                      <strong>비전 충전물 3개 미만:</strong> <SkillIcon skill={skillData.glacialadvance} textOnly={true} /> 우선 시전
                    </li>
                    <li>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    <li>
                      <strong><SkillIcon skill={skillData.breathofthe} textOnly={true} /> 효과:</strong> 마나 즉시 100% 회복 + 주문력 대폭 증가 + <SkillIcon skill={skillData.killingmachine} textOnly={true} /> 자동 부여
                    </li>
                    <li>
                      <strong><SkillIcon skill={skillData.empowerruneweapon} textOnly={true} /> 사용:</strong> 전투 4초 전 프리-풀 전용 (<SkillIcon skill={skillData.killingmachine} textOnly={true} /> + 지능 버프 + 마나 재생)
                    </li>
                    <li>
                      <strong>충전물 관리:</strong> 비전 충전물 4중첩 시 <SkillIcon skill={skillData.froststr} textOnly={true} />로 소모
                    </li>
                    <li>
                      <strong><SkillIcon skill={skillData.killingmachine} textOnly={true} /> 활용:</strong> <SkillIcon skill={skillData.howlingblast} textOnly={true} /> 마나 소모 없음 - 적극 활용
                    </li>
                  </ul>
                </div>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    ...
                    <li>
                      <strong style={{ color: '#ff6b6b' }}><SkillIcon skill={skillData.remorselesswinter} textOnly={true} /> 효과:</strong> 비전 주문이 모든 대상에게 피해 + 주문불꽃 구체 대량 생성 (20초 지속)
                    </li>
                    <li>
                    ...
                    <li>
                      <strong>우선순위 2:</strong> <SkillIcon skill={skillData.frostfever} textOnly={true} /> 없을 때 <SkillIcon skill={skillData.killingmachine} textOnly={true} /> 번뜩임으로 <SkillIcon skill={skillData.howlingblast} textOnly={true} /> 시전
                    </li>
                    <li>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    <li>
                      <strong style={{ color: '#ffa500' }}>메커니즘:</strong> <SkillIcon skill={skillData.frostscythe} textOnly={true} /> (비전의 여파) 대상에게 4% 피해 증가 (최대 4중첩 = 16%)
                    </li>
                    <li>
                ...
                <div style={{ marginBottom: '25px' }}>
                  ...
                  <ul style={{ lineHeight: '1.8' }}>
                    ...
                    <li>
                      <strong>발동 조건:</strong> <SkillIcon skill={skillData.killingmachine} textOnly={true} /> 번뜩임 소모 시 <SkillIcon skill={skillData.frostscythe} textOnly={true} /> 버프 부여
                    </li>
                    <li>
            ...
            <div style={{ marginTop: '30px' }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: '0.9rem', lineHeight: '1.8', color: '#ccc' }}>
                  ...
                  • <strong style={{ color: '#28a745' }}>위크오라 설정:</strong> 비전의 여파, 번뜩임 버프, {selectedTier === 'mountainthane' ? '비전 충전물 중첩' : renderTextWithSkillIcons('비전 보주 쿨')}, 마나 게이지 추적 필수
                </p>
              </div>
            </div>
          </div>
  ...
  const talentBuilds = {
    mountainthane: {  // 산왕 (Mountain Thane)
      'raid-single': {
        name: '레이드 단일 대상',
        description: '산왕을 활용한 단일 대상 빌드입니다. 비전의 여파와 비전 충전물 강화로 보스전에 특화되어 있습니다.',
        code: 'CwQAqjLKv2qfbjSJolSCJSkkSSJJJJJJAAAAAAAAAAAASSSikEJSSiQJJhEJSA',  // Mountain Thane 레이드 단일
        icon: '🔥'
      },
      'raid-aoe': {
        name: '레이드 광역',
        description: '산왕을 활용한 광역 빌드입니다. 비전 폭발로 다수 대상에게 강력한 광역 딜을 제공합니다.',
        code: 'CwQAqjLKv2qfbjSJolSCJSkkSSJJJJJJAAAAAAAAAAAASkkkEJSSiEJJhEJSA',  // Mountain Thane 레이드 광역
        icon: '🔥'
      },
      'mythic-plus': {
        name: '쐐기돌',
        description: '산왕을 활용한 쐐기돌 빌드입니다. 버스트 피해와 비전의 여파로 쐐기돌에 최적화되어 있습니다.',
        code: 'CwQAqjLKv2qfbjSJolSCJSkkSSJJJJJJAAAAAAAAAAAASkkSSJSSiEJJhkESA',  // Mountain Thane 쐐기돌
        icon: '🔥'
      }
    },
      ...
      <Card style={{ marginBottom: '20px' }}>
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border: `2px solid ${selectedTier === 'mountainthane' ? '#9482C9' : '#2a2d35'}`,
              borderRadius: '8px',
              color: selectedTier === 'mountainthane' ? '#9482C9' : '#94a3b8',
              fontSize: '1rem',
              fontWeight: '600',
              cursor: 'pointer',
        ...
        <div style={{ padding: '20px' }}>
          ...
          <h4 style={{
            ...
            {selectedTier === 'mountainthane' ? '산왕' : '종말의 기수'} 특성 빌드
          </h4>
      ...
      const breakpointData = {
        mountainthane: {  // 산왕 (Mountain Thane)
          single: {
            ...
            mastery: {
              breakpoints: [],
              note: '비전 충전물 중첩당 피해 증가, 산왕 특성과 시너지'
            },
            versatility: {
              breakpoints: [],
        ...
        rideroftheapocalypse: {  // 종말의 기수 (Rider of the Apocalypse)
          single: {
            ...
            mastery: {
              breakpoints: [],
              note: '비전 보주 피해 증가, 종말의 기수 특성과 시너지'
            },
            versatility: {
              breakpoints: [],
              note: '안정적인 피해 증가 및 생존력 향상'
      ...
      <Section ref={sectionRefs.stats} id="stats">
            ...
            <button
              ...
              style={{
                ...
                background: selectedStatHero === 'mountainthane' ?
                  ...
                  'rgba(255, 255, 255, 0.05)',
                border: `2px solid ${selectedStatHero === 'mountainthane' ? '#3FC6EA' : '#2a2d35'}`,
                borderRadius: '8px',
                color: selectedStatHero === 'mountainthane' ? '#3FC6EA' : '#94a3b8',
                fontSize: '1rem',
                fontWeight: '600',
                cursor: 'pointer',
                transition: 'all 0.3s ease'
        ...
        <Card style={{ marginBottom: '20px' }}>
          <div className={styles.subsection} ref={subSectionRefs['stats-priority']}>
            <h3 style={{
              color: selectedStatHero === 'mountainthane' ? '#3FC6EA' : '#4ECDC4',
              fontSize: '1.3rem',
              marginBottom: '25px',
              display: 'flex',
              alignItems: 'center',
              ...
              <span>{selectedStatHero === 'mountainthane' ? '🔥' : '✨'}</span>
              <span>{selectedStatHero === 'mountainthane' ? '산왕' : '종말의 기수'}</span>
            ...
            <div style={{
              ...
              {statPriorities[selectedStatHero][selectedStatMode].map((statKey, index) => {
                ...
                const isEqual = index > 0 &&
                  ((selectedStatHero === 'mountainthane' && selectedStatMode === 'single' && index === 2) ||
