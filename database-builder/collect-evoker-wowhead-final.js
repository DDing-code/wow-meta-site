...
async function collectEvokerSkillTranslations() {
  ...
  try {
      ...
      for (const skill of skills) {
        try {
          ...
          if (koreanName && koreanName !== 'Error' && !koreanName.includes('404')) {
            ...
            console.log(`✅ ${skill.id}: ${skill.englishName} → ${koreanName}`);
          } else {
            errors.push({
              id: skill.id,
              englishName: skill.englishName,
            ...
            console.log(`❌ ${skill.id}: ${skill.englishName} - 번역 실패`);
          }
    ...
    // 결과 저장
    const outputPath = path.join(__dirname, 'evoker-skills-verified-final.json');
    await fs.writeFile(
      outputPath,
      JSON.stringify({
        collectedAt: new Date().toISOString(),
    ...
    for (const spec of Object.values(results)) {
      for (const skill of Object.values(spec)) {
        translationMap[skill.englishName.toLowerCase().replace(/['\s]/g, '_')] = skill.koreanName;
      }
    }

    const mapPath = path.join(__dirname, 'evoker-translation-map.json');
    await fs.writeFile(mapPath, JSON.stringify(translationMap, null, 2));
    console.log(`📝 번역 매핑 테이블 저장: ${mapPath}`);
